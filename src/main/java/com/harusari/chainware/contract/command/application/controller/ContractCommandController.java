...
@Tag(name = "거래처별 제품 계약 API", description = "거래처별 제품 계약 생성, 수정, 삭제 API")
public class ContractCommandController {

    private final ContractService contractService;

    @Operation(summary = "거래처별 제품 계약 생성", description = "새로운 제품 계약을 생성합니다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "제품 계약 생성 성공")
    })
    @PostMapping
    public ResponseEntity<ApiResponse<ContractResponse>> createContract(
            @RequestBody(description = "제품 계약 생성 요청 정보")
    ...
    @Operation(summary = "거래처별 제품 계약 수정", description = "기존 제품 계약의 정보를 수정합니다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "제품 계약 수정 성공")
    })
    @PutMapping("/{contractId}")
    public ResponseEntity<ApiResponse<ContractResponse>> updateContract(
            @Parameter(name = "contractId", description = "수정할 제품 계약의 ID", example = "42")
            @PathVariable Long contractId,
            @RequestBody(description = "제품 계약 수정 요청 정보")
    ...
    @Operation(summary = "제품 계약 삭제", description = "지정한 제품 계약을 삭제합니다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "제품 계약 삭제 성공")
    })
    @DeleteMapping("/{contractId}")
    public ResponseEntity<ApiResponse<Void>> deleteContract(
            @Parameter(name = "contractId", description = "삭제할 제품 계약의 ID", example = "42")
            @PathVariable Long contractId
    ) {
